...
var Main = function() {
    ...
    var runContainerHeight = function() {
        ...
        var mapElement = $('.main-content > .container #mapElement');
        ...
        if (mainContainer.outerHeight() < mainNavigation.outerHeight() && mainNavigation.outerHeight() > $pageArea) {
            ...
            mapElement.css("min-height", mainNavigation.outerHeight() * 0.7);
        } else {
            mainContainer.css('min-height', $pageArea);
            mapElement.css("min-height", $pageArea * 0.7);

        }
        ;
        if ($windowWidth < 768) {
            mainNavigation.css('min-height', $windowHeight - $('body > .navbar').outerHeight());
            mapElement.css("min-height", $windowHeight - $('body > .navbar').outerHeight() * 0.7);
        }
    };
    ...
    var runNavigationToggler = function() {
        $('.navigation-toggler').bind('click', function() {
            if ($.cookie("clip-setting")) {
                ...
                var loadSetting = new Object;
            }
            if (!$('body').hasClass('navigation-small')) {
                $('body').addClass('navigation-small');
                loadSetting.navigation_size = "small";

            } else {
                $('body').removeClass('navigation-small');
                loadSetting.navigation_size = "large";
            }
            $.cookie("clip-setting", JSON.stringify(loadSetting));
...
//        });
        $('.panel-tools .panel-refresh').bind('click', function(e) {
            ...
            el.block({
                ...
                message: '<img src="/backend/assets/images/loading.gif" /> Just a moment...',
                css: {
                    border: 'none',
                    color: '#333',
                    background: 'none'
...
//        $('.save_style').bind('click', function() {
        if ($.cookie("clip-setting")) {
            var clipSetting = jQuery.parseJSON($.cookie("clip-setting"));
        } else {
...
//                );
    ...
    var runCustomSetting = function() {
        ...
        if ($.cookie("clip-setting")) {
            ...
            ;
            if (loadSetting.navigation_size === "small") {
                $('body').addClass('navigation-small');
            } else {
                $('body').removeClass('navigation-small');
            }
