 ...
 */
class redisSessionHandlerModel implements SessionHandlerInterface
{
     ...
     * Constructor
     */
    public function __construct($project = "CSGO")
    {
        $this->prefix = $project . "/sessions/";
    ...
    {
        $tmp = $_SESSION;
        $_SESSION = json_decode($this->redis->get($this->prefix . $id), true);
    ...
    public function write($id, $data)
    {
        ...
        $new_data = $_SESSION;
        $_SESSION = $tmp;
        $this->redis->setex($this->prefix . $id, $this->lifetime, json_encode($new_data));
        $this->redis->persist($this->prefix . $id);
